...
class SplunkHTTPEventcollectorOutputTest < Test::Unit::TestCase
  ...
  def create_driver(conf = CONFIG, tag = 'test')
    Fluent::Test::BufferedOutputTestDriver
      .new(Fluent::SplunkHTTPEventcollectorOutput, tag)
      .configure(conf)
  end
  ...
  def test_write
    ...
    assert_requested :post, 'https://localhost:8089/services/collector',
                     headers: {
                       ...
                       'User-Agent' => 'fluent-plugin-splunk-http-'\
                                       'eventcollector/0.2.0'
                     },
                     body: { time: time,
                             source: 'test',
                             sourcetype: 'fluentd',
  ...
  def test_4XX_error_retry
    stub_request(:post, 'https://localhost:8089/services/collector')
      ...
      .to_return(body: '{"text":"Incorrect data format","code":5,'\
                       '"invalid-event-number":0}', status: 400)
  ...
  def test_write_splitting
    ...
    assert_requested :post, 'https://localhost:8089/services/collector',
                     headers: { 'Authorization' => 'Splunk changeme' },
                     body: '{"time":' + time.to_s + ',"host":"","sourcetype":"'\
                           'fluentd","index":"main","source":"test","event":'\
                           '"a"}{"time":' + time.to_s + ',"host":"",'\
                           '"sourcetype":"fluentd","index":"main","source":'\
                           '"test","event":"b"}',
                     times: 1
    assert_requested :post, 'https://localhost:8089/services/collector',
                     headers: { 'Authorization' => 'Splunk changeme' },
  ...
  def test_utf8
    ...
    d.emit({
             'some' => {
               'nested' => 'ü†f-8'.force_encoding('BINARY'),
               'with' => ['ü', '†', 'f-8']
                         .map { |c| c.force_encoding('BINARY') }
             }
           }, time)
    d.run
